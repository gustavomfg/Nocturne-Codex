...
function SuggestionDialog({ suggestion, onClose, onApply, onOpenFile }: { suggestion: Suggestion; onClose(): void; onApply(): void; onOpenFile(file: string): void }) {
  ...
  const copy = async (kind: 'diff' | 'commit', content: string) => { await window.nocturne.clipboard.writeText(content); setCopied(kind); window.setTimeout(() => setCopied((current) => current === kind ? null : current), 1_600) }
