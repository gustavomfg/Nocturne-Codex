...
export function SuggestionsPanel({ suggestions, hasMore, loadingMore, onLoadMore, onStatus, onApply, onOpenFile, onNotify }: Props) {
  ...
  return <div className="suggestions-view">
    ...
    <div className="suggestion-list">{suggestions.map((suggestion) => <article className={`suggestion-card ${suggestion.severity}`} key={suggestion.id}><header><span/><b>{labels[suggestion.category]}</b><small>{severityLabel(suggestion.severity)} · confiança {suggestion.confidence}%</small></header><h4>{suggestion.title}</h4><p>{suggestion.description}</p><div className="suggestion-files">{suggestion.affectedFiles.slice(0, 4).map((file) => <button key={file} onClick={() => onOpenFile(file)}><FileCode2 size={12}/>{file}</button>)}</div><footer><em className={suggestion.status}>{statusLabel(suggestion.status)} · {new Date(suggestion.updatedAt).toLocaleDateString('pt-BR')}</em><button onClick={() => setSelected(suggestion)}><Eye size={13}/>Ver solução</button>{suggestion.status === 'new' && <button onClick={() => onStatus(suggestion, 'in-analysis')}>Analisar</button>}{!['accepted', 'resolved', 'rejected', 'invalid'].includes(suggestion.status) && <><button onClick={() => onStatus(suggestion, 'rejected')}><X size={13}/>Rejeitar</button><button className="apply" onClick={() => onApply(suggestion)}><Check size={13}/>Aplicar</button></>}</footer></article>)}{hasMore && <button className="collection-load-more" disabled={loadingMore} onClick={onLoadMore}>{loadingMore ? 'Carregando…' : 'Carregar sugestões anteriores'}</button>}</div>
