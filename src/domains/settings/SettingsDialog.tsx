...
import { Activity, Bot, Folder, Monitor, Server, Settings, Star, X, type LucideIcon } from 'lucide-react'
...
type SettingsPage = 'codex' | 'providers' | 'workspace' | 'application' | 'diagnostics'
...
export function SettingsDialog({ value, status, workspaces, onClose, onSave, onNotify, onOnboarding }: { value: CodexSettings; status: string; workspaces: Workspace[]; onClose(): void; onSave(value: CodexSettings): void | Promise<void>; onNotify(message: string): void; onOnboarding(): void }) {
  ...
  const dirty = settingsDirty || providerDirty
    ...
    <div ref={dialogRef} className="settings-dialog beta-settings" role="dialog" aria-modal="true" aria-labelledby="settings-title" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="settings-layout">
        <nav className="settings-navigation" aria-label="Seções das configurações">
          {settingsPages.map((item) => { const Icon = item.icon; const blocked = providerDirty && page === 'providers' && item.id !== 'providers'; return <button key={item.id} className={page === item.id ? 'active' : ''} aria-label={item.label} aria-current={page === item.id ? 'page' : undefined} disabled={blocked} title={blocked ? 'Salve ou cancele a edição do Provider.' : undefined} onClick={() => setPage(item.id)}><Icon size={17}/><span><strong>{item.label}</strong><small>{item.description}</small></span></button> })}
        </nav>
        <main className="settings-content">
          ...
          {page === 'providers' && <ProviderSettingsPage onDirtyChange={setProviderDirty} onNotify={onNotify}/>}
