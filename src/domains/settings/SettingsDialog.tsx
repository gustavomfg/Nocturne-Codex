...
const settingsPages: Array<{ id: SettingsPage; label: string; description: string; icon: LucideIcon }> = [
  { id: 'ai', label: 'IA', description: 'Conta ou chave de API', icon: Bot },
...
export function SettingsDialog({ value, status, workspace, workspaces, onClose, onSave, onNotify, onOnboarding, onRecheck }: { value: CodexSettings; status: string; workspace: string; workspaces: Workspace[]; onClose(): void; onSave(value: CodexSettings): void | Promise<void>; onNotify(message: string): void; onOnboarding(): void; onRecheck(): Promise<void> }) {
    ...
    <div ref={dialogRef} className="settings-dialog beta-settings" role="dialog" aria-modal="true" aria-labelledby="settings-title" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="settings-layout">
        <nav className="settings-navigation" aria-label="Seções das configurações">
          {settingsPages.map((item) => { const Icon = item.icon; const blocked = (providerDirty || modelDirty) && page === 'ai' && item.id !== 'ai'; return <button key={item.id} className={page === item.id ? 'active' : ''} aria-label={item.label} aria-current={page === item.id ? 'page' : undefined} disabled={blocked} title={blocked ? 'Salve ou cancele as alterações desta seção.' : undefined} onClick={() => setPage(item.id)}><Icon size={17}/><span><strong>{item.label}</strong><small>{item.description}</small></span></button> })}
        </nav>
        <main className="settings-content">
          ...
          {page === 'ai' && <AISettingsPage value={value} form={form} status={status} workspaceId={workspace} onChange={setForm} onProviderDirtyChange={setProviderDirty} onModelDirtyChange={setModelDirty} onNotify={onNotify} onRecheck={onRecheck}/>}
