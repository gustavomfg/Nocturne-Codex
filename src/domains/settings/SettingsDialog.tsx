...
export function SettingsDialog({ value, status, workspaces, onClose, onSave, onOnboarding }: { value: CodexSettings; status: string; workspaces: Workspace[]; onClose(): void; onSave(value: CodexSettings): void | Promise<void>; onOnboarding(): void }) {
  ...
  const copyDiagnostic = async () => { const content = await window.nocturne.diagnostics.copy(); await navigator.clipboard.writeText(content); setCopied(true); window.setTimeout(() => setCopied(false), 1_600) }
    ...
    <div ref={dialogRef} className="settings-dialog beta-settings" role="dialog" aria-modal="true" aria-labelledby="settings-title" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="settings-layout">
        ...
        <main className="settings-content">
          ...
          {page === 'diagnostics' && <SettingsSection title="Informações do sistema"><pre className="diagnostic-summary" aria-live="polite">{diagnostic}</pre><div className="diagnostic-actions"><button onClick={() => { setDiagnostic('Reiniciando Codex…'); void window.nocturne.codex.restart().then(() => setDiagnostic('Codex reiniciado com sucesso.')).catch((error) => setDiagnostic(errorMessage(error))) }}>Reiniciar Codex</button><button onClick={() => window.nocturne.diagnostics.openLogs()}>Abrir logs</button><button onClick={() => void copyDiagnostic()}>{copied ? 'Informações copiadas' : 'Copiar informações'}</button><button onClick={() => window.nocturne.data.export()}>Exportar dados</button></div></SettingsSection>}
        </main>
      </div>
      <footer className="settings-footer"><span>As alterações são aplicadas ao salvar.</span><div className="modal-actions"><button disabled={saving} onClick={onClose}>Cancelar</button><button className="primary" disabled={saving} onClick={() => void save()}>{saving ? 'Salvando…' : 'Salvar alterações'}</button></div></footer>
    </div>
  </div>
}
