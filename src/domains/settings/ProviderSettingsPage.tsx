...
export function ProviderSettingsPage({
  ...
  const testConnection = async (id: string) => {
    ...
    try {
      ...
      if (result.status === 'available') onNotify('Conexão do Provider validada.')
    } catch (failure) {
      setError(errorMessage(failure))
    } finally {
      setTestingId(null)
  ...
  return <div className="provider-settings">
    <div className="provider-settings-intro">
      <div>
        ...
        <p>Conecte endpoints compatíveis sem alterar o funcionamento do Workspace.</p>
      </div>
      <button className="provider-add" disabled={Boolean(editor)} onClick={beginCreate}>
        <Plus size={15}/>Adicionar Provider
      </button>
    </div>
    ...
    {!editor && <div className="provider-list" aria-busy={loading}>
      ...
      {!loading && !providers.length && <div className="provider-empty">
        ...
        <p>Crie um draft local ou remoto. A conexão só será usada quando você habilitar o Provider.</p>
        <button className="provider-add" onClick={beginCreate}><Plus size={15}/>Adicionar primeiro Provider</button>
      </div>}
...
function ProviderEditorForm({
  ...
  return <section className="provider-editor" aria-labelledby="provider-editor-title">
    <header>
      <div><span><Server size={16}/></span><div><strong id="provider-editor-title">{editor.id ? 'Editar Provider' : 'Novo Provider'}</strong><small>OpenAI-compatible</small></div></div>
    ...
    <div className="provider-editor-body">
      <div className="settings-columns">
        <label>Nome<input autoFocus value={configuration.displayName} maxLength={500} onChange={(event) => onChange({ displayName: event.target.value })} placeholder="Ex.: OpenRouter pessoal"/></label>
      ...
      <label>Endpoint<input type="url" value={configuration.baseUrl} maxLength={2_048} onChange={(event) => onChange({ baseUrl: event.target.value })} placeholder="https://provider.example/v1"/></label>
      <div className="settings-columns">
