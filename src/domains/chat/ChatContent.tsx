...
export const MessageBubble = memo(function MessageBubble({ message }: { message: Message }) {
  ...
  return <div className="user-row"><div className="user-message">{message.content}{!!attachments.length && <div className="message-attachments">{attachments.map((filePath) => <span key={filePath}><Paperclip size={10}/>{filePath.split(/[/\\]/).pop()}</span>)}</div>}{awareness && <details className="message-awareness"><summary><Brain size={12}/>Contexto usado · {awareness.selections.length}</summary><div>{awareness.selections.length ? awareness.selections.map((selection) => <p key={`${selection.source}-${selection.id}`}><strong>{selection.title} · {selection.relevance}%</strong><small>{selection.reason}</small></p>) : <p><small>Nenhuma memória atingiu o limiar de relevância.</small></p>}</div></details>}</div><div className="mini-avatar">G</div></div>
})
