...
export function Sidebar({ open, compact, triggerRef, conversations, hasConversations, hasMore, loadingMore, activeId, search, searchRef, workspace, workspaces, status, onClose, onNew, onSearch, onLoadMore, onConversation, onDelete, onWorkspace, onSavedWorkspace, onFavorite, onSettings }: SidebarProps) {
  ...
  return <aside id="workspace-sidebar" ref={sidebarRef} className={`sidebar ${open ? 'open' : 'collapsed'}`} aria-hidden={!open} role={compact && open ? 'dialog' : undefined} aria-modal={compact && open ? true : undefined} aria-label={compact && open ? 'Navegação do workspace' : undefined} tabIndex={-1}>
    <div className="brand"><div className="brand-mark"><img src="./nocturne.svg" alt=""/></div><span>Nocturne <b>Studio</b></span><button className="icon-button sidebar-toggle" aria-label="Recolher barra lateral" title="Recolher barra lateral" onClick={onClose}><Menu size={17}/></button></div>
