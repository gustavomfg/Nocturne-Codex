...
describe('políticas de execução', () => {
  it.each(['sudo apt update', 'git reset --hard HEAD', 'git clean -fd', 'rm -rf build'])('marca comando perigoso: %s', (command) => expect(assessCommand(command)).toMatchObject({ risk: 'dangerous', requiresApproval: true, blockedAutomatic: true }))
