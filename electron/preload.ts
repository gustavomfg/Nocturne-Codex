...
contextBridge.exposeInMainWorld('nocturne', {
  ...
  codex: {
    ...
    send: (conversationId: string, prompt: string, attachments: string[] = [], mode = 'build') => ipcRenderer.invoke('codex:send', { conversationId, prompt, attachments, mode }),
  ...
  suggestions: { list: (conversationId: string) => ipcRenderer.invoke('suggestions:list', conversationId), create: (conversationId: string, content: string) => ipcRenderer.invoke('suggestions:create', { conversationId, content }), status: (conversationId: string, suggestionId: string, status: string, result?: string) => ipcRenderer.invoke('suggestions:status', { conversationId, suggestionId, status, result }) },
