...
export interface Suggestion {
  ...
  category: SuggestionCategory; severity: SuggestionSeverity; affectedFiles: string[]; proposedChanges: string
...
export const suggestionInputSchema = z.object({
  ...
  category: z.enum(suggestionCategories), severity: z.enum(suggestionSeverities), affectedFiles: z.array(z.string().trim().min(1).max(1_000)).max(100).default([]), proposedChanges: z.string().max(100_000).default(''),
})
