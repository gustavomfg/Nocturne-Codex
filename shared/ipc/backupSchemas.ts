...
const brainMemory = z.object({ id: uuid, workspace_id: pathValue, conversation_id: uuid.nullable().default(null), kind: z.enum(['fact', 'decision', 'preference', 'constraint', 'learning']), scope: z.enum(['workspace', 'conversation']), status: z.enum(['candidate', 'active', 'outdated', 'archived']), content: z.string().trim().min(1).max(8_000), confidence: z.number().int().min(0).max(100), source_type: z.enum(['manual', 'message', 'agent']), source_id: z.string().max(500).nullable().default(null), created_at: timestamp, updated_at: timestamp, last_confirmed_at: timestamp.nullable().default(null), last_used_at: timestamp.nullable().default(null), use_count: z.number().int().min(0).default(0) }).strict()
...
export const backupSchema = z.object({ schemaVersion: z.number().int().min(1).max(DATABASE_SCHEMA_VERSION), exportedAt: timestamp.optional(), conversations: z.array(conversation).max(25_000), workspaces: z.array(workspace).max(5_000), messages: z.array(message).max(100_000), artifacts: z.array(artifact).max(50_000), memories: z.array(memory).max(5_000), brainMemories: z.array(brainMemory).max(50_000).default([]), suggestions: z.array(suggestion).max(25_000).default([]), suggestionDecisions: z.array(decision).max(50_000).default([]), settings: settings.optional() }).strict().superRefine((data, context) => {
  const totalRecords = data.conversations.length + data.workspaces.length + data.messages.length + data.artifacts.length + data.memories.length + data.brainMemories.length + data.suggestions.length + data.suggestionDecisions.length
