...
const workspace = z.object({ path: pathValue, name: z.string().min(1).max(500), favorite: z.union([z.literal(0), z.literal(1)]).default(0), created_at: timestamp, last_opened_at: timestamp }).strict()
...
const settings = z.object({ model: z.string().max(100).optional(), sandbox: z.enum(['read-only', 'workspace-write']).optional(), approvalPolicy: z.enum(['untrusted', 'on-request', 'never']).optional(), codexPath: z.string().max(1_000).optional(), diagnosticMode: z.enum(['true', 'false']).optional(), theme: z.enum(['dark', 'system']).optional(), defaultAgentMode: z.enum(agentModes).optional() }).strict()

export const backupSchema = z.object({ schemaVersion: z.number().int().min(1).max(5), exportedAt: timestamp.optional(), conversations: z.array(conversation).max(100_000), workspaces: z.array(workspace).max(10_000), messages: z.array(message).max(1_000_000), artifacts: z.array(artifact).max(1_000_000), memories: z.array(memory).max(10_000), suggestions: z.array(suggestion).max(100_000).default([]), suggestionDecisions: z.array(decision).max(1_000_000).default([]), settings: settings.optional() }).strict().superRefine((data, context) => {
  ...
  for (const [index, item] of data.artifacts.entries()) if (!conversations.has(item.conversation_id)) context.addIssue({ code: 'custom', path: ['artifacts', index, 'conversation_id'], message: 'Conversa referenciada não existe no backup.' })
  for (const [index, item] of data.suggestions.entries()) if (!conversations.has(item.conversation_id)) context.addIssue({ code: 'custom', path: ['suggestions', index, 'conversation_id'], message: 'Conversa referenciada não existe no backup.' })
